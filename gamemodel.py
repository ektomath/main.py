...
class Game:
    def __init__(self, cannonSize, ballSize):
        self.player1 = Player(self, -90, 0, 'blue')
        self.player2 = Player(self, 90, 0, 'red')
    ...
    def getCurrentPlayer(self):
        ...
        return players[self.currentplayer]

    """ The opponent of the current player """
    def getOtherPlayer(self):
        for x in self.getPlayers():
            if x != self.getPlayers()[self.currentplayer]:
                return x
...
""" Models a player """
class Player:
    def __init__(self, game, xpos, ypos, color):
        self.game = game
        self.xPos = xpos
        self.yPos = ypos
        self.color = color
    ...
    def fire(self, angle, velocity):
        ...
        player_number = self.game.getPlayers().index(self)
        if player_number == 0:
            self.projectile = Projectile(self.angle, self.velocity, self.game.getCurrentWind(), self.xPos, self.yPos+self.game.getCannonSize()/2, -110, 110)
        else:
            self.projectile = Projectile(180-self.angle, self.velocity, self.game.getCurrentWind(), self.xPos, self.yPos+self.game.getCannonSize()/2, -110, 110)
    ...
    def projectileDistance(self, proj):
        if proj.getX() > self.getX():
            if abs(-self.getX()+proj.getX()) < abs(self.game.getCannonSize()/2+self.game.getBallSize()):
                return 0
            else:
                return -self.getX()+proj.getX()-self.game.getCannonSize()/2-self.game.getBallSize()
        else:
            if abs(-self.getX() + proj.getX()) < abs(self.game.getCannonSize() / 2 + self.game.getBallSize()):
                return 0
            else:
